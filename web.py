...
st.caption("Guided Practice 1 · Question 1")

with st.sidebar:
    st.header("Question 1 parameters")

    population = st.number_input("Population", min_value=100, max_value=1_000_000, 
                                 value=10_000, step=100)

    prevalence = st.slider("Prevalence  P(I)", min_value=0.001, max_value=0.100, 
                           value=0.010, step=0.001, format="%.3f")

    sensitivity = st.slider("Sensitivity  P(+ | I)", min_value=0.00, max_value=1.00, 
                            value=0.80, step=0.01)

    specificity = st.slider("Specificity  P(− | Iᶜ)", min_value=0.00, 
                            max_value=1.00, value=0.95, step=0.01)

    step = st.slider("Increase used in Cells 1–2", min_value=0.01, max_value=0.20, 
                     value=0.04, step=0.01)

    st.divider()
    st.header("Question 2 parameters")

    songs = st.slider("Playlist size  N", min_value=5, max_value=100, value=20, step=1)

    by_artist = st.slider("Songs by the artist  K", min_value=1, max_value=songs, 
                          value=min(4, songs), step=1)

    played = st.slider("Songs counted for X  n", min_value=1, max_value=songs, 
                       value=min(10, songs), step=1)

    target = st.slider("Target artist song for Y  r", min_value=1, max_value=by_artist, 
                       value=min(2, by_artist), step=1)

    runs = st.select_slider("Simulation runs", options=[10, 100, 10_000], value=10_000)

    seed = st.number_input("Random seed", min_value=0, value=2029, step=1)
    ...
    fraction_by_artist = st.slider("Artist fraction  K/N", min_value=0.05, 
                                   max_value=0.50, value=0.20, step=0.05)

    st.divider()
    st.header("Question 3 parameters")

    rate = st.slider("Request rate  λ (per second)", min_value=0.5, 
                     max_value=10.0, value=2.0, step=0.5)

    seats = st.number_input("Number of seats", min_value=1, max_value=500, 
                            value=60, step=1)

    time_cutoff = st.slider("Time cutoff  t (seconds)", min_value=0.0, 
                            max_value=120.0, value=30.0, step=1.0)

    q3_seed = st.number_input("Question 3 random seed", min_value=0, value=2029, step=1)

    st.divider()
    st.caption(
        "Changing any control reruns the current session, "
        "so plots and tables update automatically."
    )

base_posterior = posterior(prevalence=prevalence, sensitivity=sensitivity, specificity=specificity)
...
with q2_tab1:
    ...
    result_4 = question2_playlist_experiment(songs=songs, by_artist=by_artist, played=played, 
    ...
    for event_name in result_4["event_headers"]:
        q2_column_config[event_name] = st.column_config.NumberColumn(event_name, format="%.4f")
...
with q2_tab2:
    ...
    try:
        ...
        if any(N < played for N in playlist_sizes):
            raise ValueError(
                f"Every playlist size must be at least n = {played}."
            )
...
with q3_tab3:
    ...
    st.dataframe(
        ...
        width="stretch",
        column_config={
            "simulated": st.column_config.NumberColumn(
                "Simulated",
                format="%.4f",
            ),
            "exact": st.column_config.NumberColumn(
    ...
    theoretical_col2.metric(f"E[T({int(seats)})]", f"{int(seats) / rate:.3f} s")
